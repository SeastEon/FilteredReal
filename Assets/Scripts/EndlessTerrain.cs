...
public class EndlessTerrain : MonoBehaviour
{
        ...
        for(int yOffset = -chunksVisibleInViewDistance;  yOffset <= chunksVisibleInViewDistance; yOffset++) {
            ...
            for (int xOffset = -chunksVisibleInViewDistance; xOffset <= chunksVisibleInViewDistance; xOffset++)
            {
                ...
                if (chunksDictionary.ContainsKey(viewedChunkCoord))
                {
                    ...
                    chunksDictionary.Add(viewedChunkCoord, new TerrainChunks(viewedChunkCoord, chunkSize, transform));
                }
            }
        }
    }
    ...
    public class TerrainChunks {
        ...
        Vector2 position;
        Bounds bounds;
        public TerrainChunks(Vector2 coord, int size, Transform parent)
        {
            position = coord * size;
